...
import { GET_ALL_VOTES_BY_POST_ID } from '../graphql/quries'
...
function Post({ post }: Props) {
  ...
  return (
    <Link href={`/post/${post.id}`}>
        <div className='rounded-md flex cursor-pointer border
            border-gray-300 bg-white shadow-sm hover:border hover:border-gray-600'>
            <div className='flex flex-col items-center justify-start space-y-1
            rounded-l-md bg-gray-50 p-4 text-gray'>
                <ArrowUpIcon
                    onClick={() => upvote(true)} 
                    ...
                    <p className='text-xs text-gray-400'>
                        <Link href={`/subreddit/${post.subreddit[0]?.topic}`}>
                            <span className='font-bold text-black
                             hover:text-blue-400'>r/{post.subreddit[0]?.topic}
                            </span>
                        </Link> | Posted by
                        u/{post.username} <TimeAgo date={post.created_at}/>
                    </p>
                </div>
